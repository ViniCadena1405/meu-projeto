...
describe('User Routes', () => {
  test('GET /users deve retornar todos os usuários', async () => {
    ...
    expect(res.body).toEqual([{ id: 111111111, name: 'John Doe', country: 'USA' }]);
  });
  ...
  test('POST /users deve criar um novo usuário', async () => {
    ...
    const res = await request(app).post('/users').send({ name: 'John Doe', country: 'USA' });
  ...
  test('PUT /users/:id deve atualizar um usuário', async () => {
    ...
    const res = await request(app).put('/users/1').send({ name: 'John Doe', country: 'USA' });
